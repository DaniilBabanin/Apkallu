...
Append-only on `events` is a **permission**, not a convention — plus a write-once trigger
on `invalidated_by`. Tests (Step 4) assert permission-denied. The default Ubuntu hba line
`host all all 127.0.0.1/32 scram-sha-256` covers `agency_loop` — no pg_hba/pg_ident edits.
...
- **Unix sockets are impossible in-sandbox.** seccomp denies `socket(AF_UNIX)` at the
  syscall level (EPERM before any path is involved), so no path-scoped allow can exist;
...
- **Direct TCP to host loopback is dead too** (sandbox netns) — but the sandbox ships an
  HTTP CONNECT proxy on `localhost:3128` (and SOCKS5 on `:1080`) whose allowlist includes
